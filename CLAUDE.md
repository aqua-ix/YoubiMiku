...
Two of those keys exist because of failures that were invisible before. `streamed_chars` distinguishes the two ways a streamed reply can fail — nothing arrived, or it broke off part-way — which produce the same exception (#100). `last_trim_level` is set from `Application.onTrimMemory()` at `TRIM_MEMORY_RUNNING_LOW` or worse, so a report that follows memory pressure can be told apart; a process killed outright by `lowmemorykiller` (#101) leaves no report at all, and this is the closest signal available. ANRs like the one #100 produced need no code — Crashlytics reports them from `ApplicationExitInfo` on Android 11+.
